  ...
  },
  {
    services: {
      performSignup: async (ctx, event) => {
        ...
        const resp = await httpClient.post(`${getApiBaseUrl()}/users`, payload);
        history.push("/signin");
        return resp.data;
      },
      performLogin: async (ctx, event) => {
        return await httpClient
          .post(`${getApiBaseUrl()}/login`, event)
          .then(({ data }) => {
            history.push("/");
            return data;
          })
      ...
      getUserProfile: async (ctx, event) => {
        const resp = await httpClient.get(`${getApiBaseUrl()}/checkAuth`);
        return resp.data;
      },
      ...
      updateProfile: async (ctx, event: any) => {
        ...
        const resp = await httpClient.patch(
          `${getApiBaseUrl()}/users/${payload.id}`,
          payload
        );
        return resp.data;
      },
      performLogout: async (ctx, event) => {
        ...
        return await httpClient.post(`${getApiBaseUrl()}/logout`);
      },
